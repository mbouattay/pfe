...
export class SprintParticipantsService {
  ...
  async addParticipant(
    ...
    if (actorId && !(await this.isAdmin(actorId))) {
      throw new ForbiddenException(
        'Only admins can manage sprint participants for now',
      );
    }
    ...
    let sprint = await this.prisma.sprint.findUnique({
      where: { id: sprintId },
    ...
    await this.notifications.notifyAddedToSprintChat(
      userId,
      sprintId,
  ...
  async removeParticipant(sprintId: number, userId: number, actorId?: number) {
    ...
    await this.ensureSprint(sprintId);
    await this.prisma.sprintParticipant.deleteMany({
